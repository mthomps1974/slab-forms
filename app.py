...
def add_sig_table(doc, signer_label, sig_name, date):
    ...
    r = table.rows[0].cells[0].paragraphs[0].add_run(signer_label)
    r.bold = True
    r.font.size = Pt(9)
    r2 = table.rows[0].cells[1].paragraphs[0].add_run('Date:')
    r2.bold = True
    r2.font.size = Pt(9)
    sig_run = table.rows[1].cells[0].paragraphs[0].add_run(sig_name)
    sig_run.font.size = Pt(18)
    sig_run.font.italic = True
    sig_run.font.color.rgb = RGBColor(0x0D, 0x1B, 0x2A)
    table.rows[1].cells[1].paragraphs[0].add_run(date).font.size = Pt(9)
...
def make_aa(d):
    ...
    title.add_run('Civil Advice and Assistance / Civil ABWOR Legal Aid Online Declaration').bold = True
    sub = doc.add_paragraph()
    sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
    sub.add_run('AA/LAO/CIV  |  Thompson Family Law  |  Subject: ' + v(d,'subject'))
    ...
    for name, num, typ, bal in [(v(d,'ba1n'),v(d,'ba1num'),v(d,'ba1t'),v(d,'ba1b')),(v(d,'ba2n'),v(d,'ba2num'),v(d,'ba2t'),v(d,'ba2b')),(v(d,'ba3n'),v(d,'ba3num'),v(d,'ba3t'),v(d,'ba3b'))]:
        if name or num:
            row = table.add_row()
            for cell, val in zip(row.cells, [name, num, typ, ('GBP'+bal) if bal else '']):
                cell.paragraphs[0].add_run(val).font.size = Pt(9)
    doc.add_paragraph()
    add_hdr(doc, 'E.  Income Details')
    add_field(doc, 'Non-passport Benefits:', v(d,'nonpass'))
    add_field2(doc, 'Pay/Sick Pay (weekly net):', ('GBP'+v(d,'earn_pay')) if v(d,'earn_pay') else '--', 'Self-employed drawings:', ('GBP'+v(d,'earn_se')) if v(d,'earn_se') else '--')
    add_field(doc, 'Documentary Evidence Provided:', v(d,'evidence'))
    add_hdr(doc, 'Applicant Declaration')
    doc.add_paragraph().add_run('Please read each statement carefully. By signing you confirm all information is accurate.').font.size = Pt(9)
    add_decl_items(doc, [
    ...
    add_decl_items(doc, [
        ...
        'I will retain this signed document and will send it to SLAB upon request.',
    ])
...
def make_civ(d):
    ...
    title = doc.add_paragraph()
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    title.add_run('Civil Legal Aid - Legal Aid Online Declaration').bold = True
    sub = doc.add_paragraph()
    sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
    sub.add_run('CIV/SOL  |  Thompson Family Law  |  Subject: ' + v(d,'subject'))
    add_hdr(doc, 'A.  Applicant Details')
    add_field2(doc, 'Forename:', v(d,'fname'), 'Surname:', v(d,'lname'))
    add_field2(doc, 'Date of Birth:', v(d,'dob'), 'NI Number:', v(d,'ni') or 'Not provided')
    add_field2(doc, 'Telephone:', v(d,'tel'), 'Email:', v(d,'email'))
    ...
    doc.add_paragraph().add_run('I have seen the financial information given in this declaration. It is a true statement of my personal and financial circumstances.').font.size = Pt(9)
...
def send_email(d, aa_buf, civ_buf):
    ...
    message = Mail(from_email=FROM_EMAIL, to_emails=FIRM_EMAIL, subject='New Legal Aid Declaration - ' + fname + ' ' + lname + ' - ' + ref, plain_text_content=body)
    for buf, filename in [(aa_buf, 'AA_LAO_CIV_' + lname + '_' + fname + '.docx'), (civ_buf, 'CIV_SOL_' + lname + '_' + fname + '.docx')]:
        encoded = base64.b64encode(buf.read()).decode()
        attachment = Attachment(FileContent(encoded), FileName(filename), FileType('application/vnd.openxmlformats-officedocument.wordprocessingml.document'), Disposition('attachment'))
...
def submit():
    try:
        ...
        print('Submit received:', v(d,'fname'), v(d,'lname'))
        aa_buf = make_aa(d)
        print('AA doc built')
        civ_buf = make_civ(d)
        print('CIV doc built')
